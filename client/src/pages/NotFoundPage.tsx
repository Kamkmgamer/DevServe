...
/**
 * Supernova Explosion Engine v2.0
 * Generates a cataclysmic particle explosion with screen shake,
 * chromatic aberration flash, shockwaves, and hyper-realistic physics.
 * Replaces the tame 'useConfetti'.
 */
function useSupernovaExplosion() {
  ...
  const supernova = useCallback(
    (x?: number, y?: number, power = 1) => {
      ...
      const N = Math.floor(400 * power); // More particles!
      const particles = Array.from({ length: N }, () => {
        const angle = Math.random() * Math.PI * 2;
        const speed = (Math.random() ** 2 * 12 + 8) * power; // Exponential distribution for more impactful bursts
      ...
      // Chromatic Aberration Flash & Shockwave Ring
      ...
      const animate = () => {
        ...
        if (ring.alive) {
          ...
          ctx.globalCompositeOperation = "lighter"; // For a brighter shockwave
          ctx.beginPath();
        ...
        particles.forEach((p, i) => {
          ...
          // Integrate Physics
          p.vy += 0.098; // Stronger gravity
          p.speed *= p.drag;
          p.vx = Math.cos(p.angle) * p.speed;
          p.vy += frame * 0.002 * power; // Add a bit of chaotic drift
          p.angle += p.spin;
          p.x += p.vx;
          p.y += p.vy;
          p.life--;
          ...
          // Trail
          if (i % 2 === 0) { // Only half the particles get trails for performance
          ...
          switch (p.shape) {
            case "rect":
              ctx.fillRect(-p.r, -p.r, p.r * 2.5, p.r * 1.5);
              break;
            case "line":
              ctx.fillRect(-p.r * 2, -p.r * 0.25, p.r * 4, p.r * 0.5);
              break;
            case "star":
              ctx.beginPath();
              for (let j = 0; j < 5; j++) {
...
/**
 * Cosmic Cursor v1.5
 * Leaves a trail of stellar plasma in the cursor's wake.
 * An upgrade to the quaint 'useCursorComets'.
 */
function useCosmicCursor() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const count = 2; // Spawn multiple particles for a thicker trail
...
const NotFoundPage = () => {
  ...
  const { canvasRef, supernova, isShaking } = useSupernovaExplosion();
  const { containerRef: cosmicCursorRef } = useCosmicCursor();
  ...
  const [stars] = useState(() =>
    Array.from({ length: 200 }, () => ({
      ...
      parallax: Math.random() * 0.4 + 0.1, // Parallax depth factor
    }))
  );
  ...
  // Keyboard Shortcuts: H -> home, B -> Black Hole, G -> GOD MODE
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (key === "b") {
        setVortex(true);
        setTimeout(() => setVortex(false), 2000); // Vortex duration
      }
      if (key === "g") {
        supernova(window.innerWidth / 2, window.innerHeight / 2, 3.0); // GOD MODE
      }
    };
  ...
  return (
    <main
      ...
      style={{
        perspective: "1200px",
        transform: "translateZ(0) rotateX(var(--camX, 0deg)) rotateY(var(--camY, 0deg))",
        transformStyle: "preserve-3d",
      }}
    >
      {/* Cursor plasma trail container */}
      <div ref={cosmicCursorRef} className="pointer-events-none absolute inset-0 z-50" />

      {/* Nebula & Aurora Layers */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 mix-blend-plus-lighter opacity-80"
        style={{
          background:
            "radial-gradient(120% 100% at 50% 0%, rgba(56,189,248,0.15), transparent 50%), " +
            "radial-gradient(110% 100% at 80% 20%, rgba(192,132,252,0.12), transparent 50%), " +
            "radial-gradient(100% 100% at 20% 60%, rgba(244,63,94,0.12), transparent 50%)",
      ...
      <div
        ...
        style={{
          background: "repeating-conic-gradient(from 0deg, rgba(255,255,255,0.02) 0deg 5deg, transparent 5deg 10deg)",
          mixBlendMode: "soft-light",
          animation: "nebulaSpin 80s linear infinite",
        }}
      />
      ...
      <div ref={starfieldRef} className="absolute inset-0 transition-transform duration-300 ease-out" style={{ transform: "translate(var(--mouseX, 0), var(--mouseY, 0))", transformStyle: "preserve-3d" }}>
        {stars.map((s, i) => (
          <span
            key={i}
            className="absolute rounded-full bg-slate-300/80 shadow-[0_0_12px_rgba(255,255,255,0.8)]"
            style={{
              left: `${s.x}%`,
              top: `${s.y}%`,
              width: s.s,
              ...
              transform: `translateZ(${s.parallax * -200}px)`, // This creates the 3D effect
            }}
          />
        ))}
      </div>
      ...
      <div className="absolute inset-0 overflow-hidden">
        {shooters.map((s, i) => (
          <span
            key={s.key}
            className="absolute h-0.5 w-48 -translate-x-full bg-gradient-to-r from-transparent via-purple-300 to-transparent opacity-0"
            style={{
              top: `${s.top}%`,
              left: "-10%",
              filter: "drop-shadow(0 0 10px rgba(200,200,255,0.8)) drop-shadow(0 0 24px rgba(168,85,247,0.7))",
              animation: `shoot ${s.dur}s ease-in ${s.delay + i * 0.8}s infinite`,
            }}
          />
        ))}
      </div>

      {/* Floating Blobs of Cosmic Dust */}
      <div className="absolute inset-0 overflow-hidden">
        {particles.map((p, i) => (
          <span
            key={i}
            className="absolute rounded-full opacity-40 blur-lg mix-blend-screen"
            style={{
              left: `${p.x}%`,
              top: `${p.y}%`,
              width: p.size,
              height: p.size,
              backgroundColor: `hsl(${p.hue}deg 100% 70%)`,
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[60vh]">
        ...
        <div
          className="absolute inset-0 opacity-80 [transform:perspective(1000px)_rotateX(70deg)_translateY(45%)] [transform-origin:bottom_center]"
          style={{
            backgroundImage:
            ...
            boxShadow: "inset 0 0 80px rgba(59,130,246,0.3), 0 0 100px rgba(59,130,246,0.25)",
          }}
        />
        <div
          className="absolute inset-0 [transform:perspective(1000px)_rotateX(70deg)_translateY(45%)] [transform-origin:bottom_center]"
          style={{
            background: "radial-gradient(60% 80% at 50% 100%, rgba(129,140,248,0.2), transparent 70%)",
      ...
      <div className="pointer-events-none absolute inset-0 z-20 bg-[url('/scanlines.png')] opacity-20 mix-blend-overlay"></div>
      ...
      <section className="relative mx-auto flex max-w-6xl flex-col items-center px-4 py-16 text-center sm:py-20 md:py-28 z-10">
        ...
        {/* Orbital Emojis and Glitched 3D Headline */}
        <div className="relative mt-8 grid place-items-center">
          <h1
            ref={titleRef}
            className="glitch-text relative z-10 text-7xl font-black tracking-tighter md:text-8xl"
            data-text="404"
            style={{
              textShadow:
                "0 0 24px rgba(167,139,250,0.6), 0 0 48px rgba(236,72,153,0.4), 0 0 1px rgba(255,255,255,0.8)",
          ...
          <div className="pointer-events-none absolute inset-0">
            {emojiRing.map((em, i) => (
              <span
                key={i}
                className="absolute select-none text-2xl md:text-4xl"
                style={{
                  left: "50%",
                  top: "50%",
                  transformOrigin: "0 -160px", // Increased orbit radius
                  transform: `translate(-50%,-50%) rotate(${(360 / emojiRing.length) * i}deg)`,
                  animation: `orbit 12s linear infinite`,
                  animationDelay: `${i * -1.0}s`, // Reverse direction
                  filter: "drop-shadow(0 0 8px rgba(255,255,255,0.7)) drop-shadow(0 0 20px rgba(236,72,153,0.6))",
                }}
              >
        ...
        {/* Hyper-Styled Buttons */}
        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          <Link
            ref={btnRef}
            to="/"
            onMouseMove={(e) => { onMagnetic(e); onPointerGlow(e); }}
            onMouseLeave={onMagneticLeave}
            className="group relative inline-flex items-center gap-2.5 overflow-hidden rounded-xl border border-blue-500/50 bg-gradient-to-br from-blue-600 to-indigo-600 px-6 py-3 text-base font-bold text-white shadow-[0_10px_40px_-10px_rgba(59,130,246,0.8)] transition-transform duration-150 ease-out hover:scale-[1.04] active:scale-[0.97]"
            style={{
                boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.15), 0 10px 40px -10px rgba(59,130,246,0.8)",
                filter: "drop-shadow(0 0 12px rgba(59,130,246,0.7)) drop-shadow(0 0 30px rgba(59,130,246,0.5))",
            }}
          >
            <span ref={glowRef} className="pointer-events-none absolute inset-0 -z-10 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
              style={{ background: "radial-gradient(160px 120px at var(--x,50%) var(--y,50%), rgba(255,255,255,0.35), transparent 70%)", }}
            />
      ...
      <style>{`
        ...
        .glitch-text {
          ...
          background: linear-gradient(135deg, white 40%, rgba(255,255,255,0.2));
        ...
        @keyframes glitch {
          0%, 100% { transform: translate(0, 0); clip-path: inset(45% 0 35% 0); }
          25% { clip-path: inset(15% 0 55% 0); }
          50% { transform: translate(0.5em, -0.1em); clip-path: inset(65% 0 15% 0); }
        ...
        @keyframes screenShake {
          ...
          10%, 30%, 50%, 70%, 90% { transform: translate(-2px, 3px) rotate(-0.5deg); }
        ...
        @keyframes vortexSpin {
          0% { transform: scale(1) rotate(0deg); filter: blur(0px) saturate(1) contrast(1) hue-rotate(0deg); }
          50% { transform: scale(1.1) rotate(15deg); filter: blur(4px) saturate(2.5) contrast(1.5) hue-rotate(90deg); }
          90% { transform: scale(0.1) rotate(-180deg); filter: blur(16px) saturate(5) contrast(2) hue-rotate(360deg) invert(1); }
          100% { transform: scale(1) rotate(0deg); filter: blur(0px) saturate(1) contrast(1) hue-rotate(0deg); }
        }
        ...
        @keyframes shoot {
          ...
          5% { opacity: 1; }
          80% { opacity: 1; }
          100% { transform: translateX(140vw) translateY(-25vh) rotate(10deg); opacity: 0; }
        }
